...
- [ ] T001 Add the publisher catalogue to `instructions/pictograms.md` — ARASAAC's
...
- [ ] T002 Parse it in `app/packages/core/src/pictograms/publisher.ts`, with a guard in
...
- [ ] T003 `app/packages/core/src/pictograms/wordlist.ts`: build a word list from
  material, **excluding every name Rampa knows** (FR-2109, `018` FR-1610). Reuses
  `009`'s name machinery rather than a second list.
- [ ] T004 [P] Test in `app/packages/core/test/pictogram-fetch.test.ts`: a word list built
  from material containing a learner's name, code and school contains none of them
  (SC-2104, FR-2108).
- [ ] T005 `app/packages/core/src/pictograms/fetch.ts` — `planFetch(words, present,
...
- [ ] T006 `readCandidates(json, publisher)` in the same file: publisher response →
  `{ id, keywords }[]`, **every** candidate kept (FR-2113, so `018` FR-1609 can still
  turn ambiguity into an omission).
- [ ] T007 `mergeSet(existing, fetched)`: the metadata to write, in `018`'s exact
  format, idempotent so a resumed fetch cannot duplicate an entry (FR-2102, FR-2115).
- [ ] T008 [P] Test: `mergeSet`'s output is read by `018`'s **unmodified** `readSet` with
...
- [ ] T009 `app/packages/shell/src/ipc/pictograms.ts`: record and read the acceptance —
  what was accepted, from whom, and when (FR-2104).
- [ ] T010 Withdrawal: stops further fetching, deletes nothing already hers (FR-2106).
- [ ] T011 `app/ui/src/pictograms/LicenceStep.tsx`: the acceptance screen — author, owner,
...
- [ ] T012 [P] Test in `app/packages/shell/test/pictogram-fetch.test.ts`: the transport is
...
- [ ] T013 `app/packages/shell/src/pictograms/download.ts`: the transport — one word per
  request, timeout, interruptible, writing atomically so an interrupted fetch leaves a
  readable set (FR-2108, FR-2115, FR-2118).
- [ ] T014 `pictograms:fetch` handler with progress, reporting found, missing by name,
  already-present, and a bound reached (FR-2114, FR-2117).
- [ ] T015 The button in `app/ui/src/pictograms/PictogramSetSection.tsx`, beside — not
  instead of — the folder picker (FR-2103): a set she assembled by hand keeps working
  with no network.
- [ ] T016 Failure in her language for offline, slow and refused, resuming at the cost of
  what is missing (US1 scenarios 3 and 5).
- [ ] T017 [P] Test: an offline transport leaves no half-written set and the second attempt
  fetches only what is missing (SC-2105).
- [ ] T018 [P] e2e in `app/e2e/pictograms.spec.ts` against a local fake publisher: accept,
  fetch, render a sheet with its attribution (SC-2101).

## Phase 5: Retiring FR-1601's over-reach

- [ ] T019 Narrow `018` FR-1601 in place, pointing at `023` — bundling and redistributing
  stay forbidden, fetching at her request does not (FR-2101).
- [ ] T020 Delete the four comments asserting there is no download and that a sheet
...
- [ ] T021 Assert no release artefact contains a pictogram (FR-2101) — the half of
  FR-1601 that survives, now with a test instead of a promise.
- [ ] T022 Backlog entry: why the download was forbidden, what reading it, and the
  legal-review item under G28 now covering it.

## Phase 6: Polish

- [ ] T023 `019`'s linear renderings and `022`'s diagrams are unaffected — asserted, not
  assumed.
- [ ] T024 Look at it in two widths and at `xlarge` (`013` FR-1113/FR-1118).
- [ ] T025 SC-2106: hand-assembled set, no network, nothing downloaded — `018`'s promise,
  still true.
