...
- [x] T001 Copiar `docs/screenshots/latest/` a `docs/screenshots/041-antes/`. (SC-3901)
- [x] T002 [US4] En `app/scripts/screenshot.mjs`, fotografiar la **pantalla de revisión**
...
- [x] T003 [US4] En `screenshot.mjs`, capturar estados: foco visible (`Tab` ×3 en el
      ...
      - **Error y carga no se han podido fotografiar**, y el guion lo dice al correr:
        `contextBridge` congela el puente, así que `learners.list` no se deja sustituir;
        y una carpeta bajo `/dev/null` no produce un error visible (la aplicación no
        llega a cargar el alumnado con ella). Los dos estados existen en `Loaded.tsx` y
        se ven en `e2e`; quedan como el hueco declarado de SC-3905.
- [x] T004 [US4] En `screenshot.mjs`, la pantalla de referencia («Quién es») en los **ocho**
...
- [x] T005 [US4] `app/e2e/a11y.spec.ts`: el barrido pasa de 4 a 8 cruces (añade
      `data-contrast="high"` al producto). (FR-3920, SC-3904)
- [x] T006 Correr `npm run shots` sobre la interfaz **actual** y copiar también esas capturas
...
- [x] T007 [US2] `tokens.css`: `--control-h`, `--control-h-sm`, `--weight-title`,
...
- [x] T008 [US1] `tokens.css` base: **un solo `h2`** (`text-lg`, `--weight-heading`), `h3`
...
- [x] T009 [US2] `components.css` · botones: `.btn` sin sombra y con `--control-h`;
      `.btn-sm` con `--control-h-sm`; `.btn-lg` se queda para los tres primarios del
      momento grande (`AdaptScreen` ×2, `VerifyScreen`) y lo pierden `ConnectStep`,
      `Walkthrough` e `IngestScreen`, donde era peso sin motivo; `:disabled` con
      colores explícitos; `[aria-busy]` **conserva** el anillo dibujado: es un trazo, no
      un glifo, y un botón con la etiqueta transparente no tiene dónde poner un hijo.
      Reescribir el comentario «Minimum 44px (FR-804)» con la razón nueva. (FR-3907,
      FR-3911, FR-3914)
- [x] T010 [US2] `components.css` · superficies: `.card` plana; `.card-object`;
      `.card-action` = objeto pulsable; retirar `.card-plain`, `.card-head`, `.eyebrow`.
      (FR-3909, FR-3914)
- [x] T011 [US2] `components.css` · avisos y marcas: `.callout` sin sombra, franja 4 px,
...
- [x] T012 [US2] `components.css` · monoespaciada: `.meta`, `.tag`, `.steps::before`,
...
- [x] T013 [US2] `components.css` · carril: `[aria-current]` con `--accent-soft`,
...
- [x] T014 [US1] `composition.css` · las clases del shell: `.row-split`, `.row-baseline`,
...
- [x] T015 [US1] `shell/Page.tsx`: `variant: 'wide' | 'narrow'`. (FR-3901)
- [x] T016 [US2] `packages/core/test/contrast.test.ts`: las nueve parejas ★ del sistema §9
      entran en `PAIRINGS` con su `why`. Debe pasar sobre los tokens de T007 sin retocar
      ningún hex. (FR-3913, SC-3903)
- [x] T017 [US1] `ui/test/styles.test.tsx`: (a) los segmentos literales de una plantilla
...
- [x] T018 [US1] `Notice` → `Callout` en sus diez usos (`ProfileEditor`, `InjectionNotice`,
      ...
      (FR-3903, FR-3904)
- [x] T019 [US1] `learners/ProfileEditor.tsx` + `learners/LearnerSections.tsx` (case `who`):
...
- [x] T020 [US1] `learners/AxisEditor.tsx`, `VehicularMark.tsx`, `YearPicker.tsx`: `strong`
...
- [x] T021 [US1] `App.tsx` onboarding: `<main className="main">` + `Page variant="narrow"`
...
- [x] T022 [US1] `onboarding/ConnectStep.tsx`: un `Page` por etapa; el `fieldset.fieldset`
...
- [x] T023 [US1] Barrido de los estilos en línea restantes, fichero a fichero, con la
      ...
      pasa a verde con **tres** excepciones, no cuatro: `Logo.tsx` tenía dos de layout y
      una de dato, y las dos de layout eran `.row-bottom` y `.gap1`. (FR-3903, SC-3902)
- [x] T024 [US1] Las 26 tarjetas: las nueve objeto de research R4 ganan `.card-object`
...
- [x] T025 [US2] `components/Callout.tsx`: el cuerpo en `<div className="callout-body">`;
      `callout.test.tsx` sigue verde. `components/Badge.tsx`: sin glifos hasta T027; sans.
      (FR-3910)
- [x] T026 [US2] `settings/DisplayPreferences.tsx`: el bloque desplegado sale del ancho del
...
- [x] T027 [US3] `components/Icon.tsx` + `components/LICENSE-lucide.txt`: los 30 trazos del
...
- [x] T028 [US3] Glifos fuera: `Badge.tsx`, `Field.tsx`, `DraftMark.tsx` (solo el `✓`),
...
- [x] T029 [US3] Iconos con palabra: los siete apartados del alumno y los seis de
...
- [x] T030 [US5] `composition.css`: el carril en `@container window (max-width: 52em)` pasa
...
- [x] T031 [US5] `e2e/layout.spec.ts`: a 560 y 880 el `.rail` mide **una fila** (su altura
...
- [x] T032 `npm test` y `npm run test:e2e` completos; anotar los números en
      `specs/006-desktop-app/validation.md` junto a los del baseline. (SC-3906)
- [x] T033 Agente de contexto fresco contra `contracts/states.md`: para cada celda rellena,
...
- [x] T034 Comparar `docs/screenshots/041-antes/` con `latest/` pantalla a pantalla y
      escribir en `validation.md` qué cambió y qué no, en frases completas. (SC-3901)
- [x] T035 BACKLOG: las dos observaciones de contenido del diagnóstico (recetas con acentos
