...
- [ ] T041 Run quickstart §5 — connect all six for real, once — and record in `specs/006-desktop-app/validation.md` which services were actually reached. An adapter implemented against documentation and never called is not verified, and that document exists to say so **(NOT done — needs six real accounts, two with money in them. Recorded in `specs/006-desktop-app/validation.md`: every adapter here was written against documentation and a stub, and an adapter never called is not verified.)** **Vindicated 2026-09-01.** Carlos ran the application and Google answered 404: the code was calling `gemini-2.0-flash`, which the vendor had retired, while the corpus correctly said `gemini-2.5-flash`. 1.245 unit tests and 76 e2e could not see it, because **every test in this project stubs the provider** — correctly, but it means this seam is exercised only by fixtures that agree with the code. This task is the only test that was ever going to find it. G24.
...
| FR-704 | `releases.test.ts` and `compatible.test.ts` — a new corpus file and no code, unless the API is not one an existing adapter covers. **And `corpus-model-reaches-provider.test.ts` since 2026-09-01**: `providerFor` dropped the corpus's `model` for the `google` and `anthropic` adapters, so `instructions/providers/google.md` said `gemini-2.5-flash` while the code called `gemini-2.0-flash`, which Google had shut down. A corpus field nobody read — the tenth in this project, and backlog **G24**. The test now asserts it for every entry that declares a model, and over the request rather than the field |
...
| FR-710 | `ServiceComparison` renders one column per fact in the requirement — card, free tier and its limit, cost per sheet, where it is processed, what the terms say about training, whether photographs work, and the plain sentence — all read from `instructions/providers/*.md` rather than written here. `e2e/connect.spec.ts` («the comparison shows every column that decides it, with dates») walks the rendered table, and `catalogue.test.ts` fails if a shipped service is missing one of the fields |
...
| FR-721 | Five distinct outcomes, each with its own sentence and next step: `key-invalid`, `key-wrong-provider`, `key-no-credit`, `offline`, `rate-limited`. `degradation.test.ts`. **Six now**, and until 2026-09-01 a teacher could see none of them: `ProviderError` was not a `RampaError`, so every provider failure crossed the IPC boundary as `unknown` and the interface said «algo ha ido mal» — the sentences existed and were unreachable (G24 B). `error-kind-survives.test.ts` holds the round trip. The sixth is `provider-model-gone`, which is deliberately **not** retryable |
