...
| FR-1205 | `RecordEntry.documents`: the source, the IR, the adapted sheet, its revisions and every render. `016` T006 made `adapted` optional, so a composed job appears before it is adapted. **`rendered` was written, typed and read by nothing until 2026-09-04** (review FLU-04, the enésima instance of G36): the record row offered the markdown, the report, the IR and a reuse, and **not the PDF she photocopies** — so reprinting a sheet from last month, which is one of the commonest tasks of the year, was impossible. The only «Guardar como PDF» lived on the review screen, unreachable for an old job until `0.1`. The row now opens the PDF it has and prints one where it has none, both offline, which is what `es.errors.offline` had been promising all along |
| FR-1209 | `record-erasure.test.ts`: the plan names the record, the adaptations, the renders and the index, and `verifyForgotten` searches the whole vault rather than trusting a delete list. **Extended 2026-09-04 to the five residues it did not reach** (P38) — the encrypted name map, the handover packets, the roster row, the composition requests and the archived journal |
| FR-1211 | `planForget` returns `sharedKept: Array<{ job, alsoUsedBy }>` and `ForgetLearner` **shows it**, by job and with the reason, before she confirms. **Until 2026-09-04 the screen's own `Plan` interface omitted the field** (review COD-20, decision P45), so no JSX could render it and what reached her was the aggregate sentence in `survives`: «N materiales se quedan…». The *how many*, never the *which* — half a requirement living only in the type of the main process. Counts and never codes: naming another child inside a dialogue about erasing this one is exposure that buys nothing, and `e2e/erasure.spec.ts` asserts no other learner's code appears |
