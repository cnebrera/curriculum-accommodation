...
| FR-807 | **Corregido 2026-09-10 · sólo media fila era verdad.** Comparten la tipografía: `render/html.ts:117` nombra `"Atkinson Hyperlegible"`, la misma que `tokens.css:173`, y desde `076` la incrusta. **La paleta no.** «`render/html.ts` derives from the same tokens» era falso: los siete colores de la hoja (`#111`, `#fff`, `#0b5f5b`, `#c9d2d1`, `#f4f7f7`, `#8a2f2c`, y el `rgba` de la marca de agua) son literales en ese fichero y ninguno aparece en `tokens.css`. Comprobado por grep, no por lectura. El requisito queda **parcialmente satisfecho**, y la mitad que falta la recoge `040` — que es la spec que toca la presentación de la hoja y donde una paleta compartida se puede decidir en vez de afirmar |
