...
- [ ] T001 Write `app/packages/core/test/sheet-structure.test.ts` and **assert the defect
...
- [ ] T002 [P] In the same file, the properties that hold **today** and must keep holding
...
- [ ] T003 [US1] Turn T001's baseline into the requirement: a heading block renders as a
...
- [ ] T004 [P] [US1] Same file: several headings render at **one level** (FR-3502) and in
...
- [ ] T005 [US1] One branch in `renderBlock`, in `app/packages/core/src/render/html.ts`:
...
- [ ] T006 [US2] Write `app/e2e/sheet-a11y.spec.ts`: render a real fixture in the three
...
- [ ] T007 [P] [US2] Same spec: a sheet carrying pictograms keeps a description on every
...
- [ ] T008 [US3] Extend `sheet-structure.test.ts` with the heading-structure properties as
...
- [ ] T009 [US3] **The mutation, as a written task** (SC-3503, quickstart §5): with heading
      blocks rendered as paragraphs again, the **structural** layer must fail and the
      **conformance** layer must stay green. Run it and record both outcomes in the task.
      That asymmetry is this specification's whole argument — if both went red one layer is
      redundant, if both stayed green the suite is theatre

---

## Phase 6: Polish

- [ ] T010 [P] Fix the comment that hid this for weeks: `app/e2e/a11y.spec.ts` excludes the
...
- [ ] T011 [P] BACKLOG: close **G7** («no conformance target for its own HTML output, and
...
- [ ] T012 [P] Look at a rendered sheet with headings, in the viewer and printed, at the
