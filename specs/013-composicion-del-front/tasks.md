...
- [x] T004 [P] Build `Section`, `Field` and `Actions`. `Actions` enforces one primary control (FR-1105) *(done. `Actions` takes a singular `primary` prop, which is how FR-1105 is enforced rather than requested — **for the screens that use `Actions`**, which is not all of them. Backlog G31 recorded that no test counted `.btn-primary` per screen and that `023` broke the rule within an hour of adding two buttons, caught by a screenshot rather than by anything that could fail. `e2e/primary-control.spec.ts` (2026-09-04, decision P35) now walks every rail destination, every section inside a learner, the steps of preparing something, and both of the questions the adapt screen asks — at 1366px and at 900px with `xlarge` text, which is the width and scale where it actually broke. It found one violation on its first run, in a notice added twenty minutes earlier.)*
